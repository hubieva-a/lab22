...
def add_window():
    ...
    lb2 = Label(add_w, text="Класс")
    lb3 = Label(add_w, text="Оценки")
...
def update_window():
    def upd():
        ...
        if var.get() == 0:
            ...
            col = 'year'
        print(type(col))
        cur.execute(f'''UPDATE peoples SET {col} = {value} WHERE _id = {row}''')
        db.commit()
    ...
    rb2 = Radiobutton(upd_w, text="Класс", indicatoron=0, variable=var, value=1)
    rb3 = Radiobutton(upd_w, text="Оценка 1",  variable=var, indicatoron=0, value=2)
    rb4 = Radiobutton(upd_w, text="Оценка 2",  variable=var, indicatoron=0, value=3)
    rb5 = Radiobutton(upd_w, text="Оценка 3",  variable=var, indicatoron=0, value=4)
...
def del_window():
    def row_del():
        row = int(en1.get())
        print(row)
        cur.execute(f'''DELETE FROM peoples WHERE _id = {row}''')
        db.commit()
...
def select_window():
    def choice():
        try:
            ...
            if res:
                ...
                text.delete(0.0, END)
                text.insert(0.0, 'Такого номера нет')
        except(ValueError, TypeError):
            mb.showinfo("Выбор номера",
                        "Введите номер!")
    ...
    lb4 = Label(sel_w, text="Введите номер")
    en4 = Entry(sel_w)
...
if __name__ == '__main__':
    ...
    file_menu.add_command(label="Сброс", command=lambda: people.kill())
